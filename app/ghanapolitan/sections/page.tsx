...
// Custom Badge Component with beautiful colors
const SectionBadge = ({ 
  children, 
  variant = 'default',
  size = 'md',
...
// Use the updated Section interface from your sectionTypes
interface Section {
  _id: string;
  section_name: string;
  section_code: string;
...
export default function SectionsPage() {
  ...
  const handleMoveOrder = async (id: string, direction: 'up' | 'down') => {
    // Note: You'll need to implement updateSectionOrder mutation
  ...
  return (
    <div className="min-h-screen p-4 md:p-6 bg-transparent">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
              Total: {sectionsData?.total || 0} sections
            </p>
          </div>
        ...
        {sectionsData?.data?.sections?.length === 0 ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-[#e0e0e0] dark:divide-neutral-800">
                    {sectionsData?.data?.sections?.map((section: Section) => (
