...
import { useUpdateOpinionMutation, useGetSingleOpinionQuery } from '@/store/features/ghweb/opinion/opinionAPI';
...
export default function EditOpinionArticlePage() {
  ...
  const opinionId = Array.isArray(params.slug) ? params.slug[0] : params.slug;
  const { data: opinion, isLoading: isOpinionLoading, error } = useGetSingleOpinionQuery(opinionId as string, {
    skip: !opinionId,
  });
  ...
  useEffect(() => {
    if (opinion && !initialLoadComplete.current) {
      setTitle(opinion.title || '');
      setDescription(opinion.description || '');
      setLabel(opinion.label || '');
      setCreator(opinion.creator || '');

      let parsedTags: string[] = [];
      if (opinion.tags) {
        if (Array.isArray(opinion.tags)) {
          parsedTags = opinion.tags.map(String);
        } else if (typeof opinion.tags === 'string') {
          try {
            const tempTags = JSON.parse(opinion.tags);
            parsedTags = Array.isArray(tempTags) ? tempTags.map(String) : [String(tempTags)];
          } catch (e) {
            parsedTags = [String(opinion.tags)];
          }
        }
      }
      setTags(parsedTags);

      setEditorContent(opinion.content || '');
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    } else if (opinion?.image_url && thumbnailPreview === opinion.image_url) {
      payload.append('image_url', opinion.image_url);
    }

    try {
      await updateOpinion({ slug: opinionId, formData: payload }).unwrap();
      router.push('/ghweb/opinions');
      notify('Opinion article updated successfully', 'success');
    } catch (err: any) {
  ...
  if (error) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <p className="text-red-600 dark:text-red-400">Error loading opinion</p>
      </div>
    );
  }
