...
import { useUpdateFeatureMutation, useGetFeatureByIdQuery } from '@/store/features/ghweb/feature/featureAPI';
...
export default function EditEditorialPage() {
  ...
  const featureId = Array.isArray(params.slug) ? params.slug[0] : params.slug;
  const { data: feature, isLoading: isFeatureLoading, error } = useGetFeatureByIdQuery(featureId as string, {
    skip: !featureId,
  });
  ...
  useEffect(() => {
    if (feature && !initialLoadComplete.current) {
      setTitle(feature.title || '');
      setDescription(feature.description || '');
      setLabel(feature.label || '');
      setCreator(feature.creator || '');
      setVenue(feature.venue || '');
      setRating(feature.rating?.toString() || '');

      let parsedTags: string[] = [];
      if (feature.tags) {
        if (Array.isArray(feature.tags)) {
          parsedTags = feature.tags.map(String);
        } else if (typeof feature.tags === 'string') {
          try {
            const tempTags = JSON.parse(feature.tags);
            parsedTags = Array.isArray(tempTags) ? tempTags.map(String) : [String(tempTags)];
          } catch (e) {
            parsedTags = [String(feature.tags)];
          }
        }
      }
      setTags(parsedTags);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    } else if (feature?.image_url && thumbnailPreview === feature.image_url) {
      payload.append('image_url', feature.image_url);
    }

    try {
      await updateFeature({ slug: featureId, formData: payload }).unwrap();
      router.push('/ghweb/features');
      notify('Feature article updated successfully', 'success');
    } catch (err: any) {
  ...
  if (error) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <p className="text-red-600 dark:text-red-400">Error loading feature</p>
      </div>
    );
  }
