...
export default function ArticlesPage({ initialArticles }: ArticlesPageProps) {
  ...
  const { 
    ...
    statusFilter !== 'all' ? { 
      ...
      ...(livescoreFilter !== 'all' && { 
        hasLivescore: livescoreFilter === 'withLivescore',
        ...(livescoreFilter === 'withLivescore' && { livescoreTag: 'active' })
      })
    } : { status: '', page: 1, limit: 0 },
    { 
      skip: statusFilter === 'all' || !!debouncedSearchTerm,
      refetchOnMountOrArgChange: true 
    }
  );
  ...
  const { 
    ...
    skip: !!debouncedSearchTerm || statusFilter !== 'all'
  });
  ...
  const handleView = (slug: string) => {
    window.open(`/ghanascore/article-detail/${slug}`, '_blank');
  };
  ...
  const clearFilters = () => {
    ...
    setCategoryFilter('');
    setStatusFilter('all');
    setLivescoreFilter('all');
    setPage(1);
  };

  const hasActiveFilters = () => {
    return searchTerm || categoryFilter || statusFilter !== 'all' || livescoreFilter !== 'all';
  };
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
              {debouncedSearchTerm ? (
                ...
                <>
                  ...
                  {livescoreFilter !== 'all' && ` • Livescore: ${livescoreFilter === 'withLivescore' ? 'With Livescore' : 'Without Livescore'}`}
                </>
              )}
            </p>
          </div>
          ...
          <div className="flex gap-3 w-full md:w-auto">
            <Link href="/ghanascore/create-article" passHref>
              <Button
        ...
        <div className="bg-white dark:bg-neutral-900 border border-[#e0e0e0] dark:border-neutral-800 rounded-lg shadow-sm p-4 mb-6">
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <div className="relative">
                <TrendingUp size={16} className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 dark:text-gray-500" />
                <select
                  value={livescoreFilter}
                  onChange={(e) => {
                    setLivescoreFilter(e.target.value);
                    setPage(1);
                  }}
                  disabled={!!debouncedSearchTerm}
                  className="pl-10 pr-3 py-2 border border-[#e0e0e0] dark:border-neutral-700 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 dark:bg-neutral-800 dark:text-gray-100 appearance-none disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <option value="all">All Livescore</option>
                  <option value="withLivescore">With Livescore</option>
                  <option value="withoutLivescore">Without Livescore</option>
                </select>
              </div>
              ...
              {debouncedSearchTerm 
                ...
                : categoryFilter
                  ...
                  : statusFilter !== 'all'
                    ...
                    : livescoreFilter !== 'all'
                      ? `No articles with ${livescoreFilter === 'withLivescore' ? 'livescore' : 'no livescore'}`
                      : hasActiveFilters() 
              ...
              {debouncedSearchTerm
                ...
                : categoryFilter
                  ...
                  : statusFilter !== 'all'
                    ...
                    : livescoreFilter !== 'all'
                      ? 'Try a different livescore filter'
                      : hasActiveFilters() 
                      ...
                      <tr key={article._id} className="hover:bg-gray-50 dark:hover:bg-neutral-800/50">
                          ...
                          <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center gap-3 text-xs text-gray-500 dark:text-gray-400">
                                ...
                                {article.hasLivescore && (
                                  <Badge variant="info" size="sm" icon={TrendingUp}>
                                    Livescore: {article.livescoreTag || 'Active'}
                                  </Badge>
                                )}
                              </div>
                            </div>
                        ...
                        <td className="py-4 px-4">
                          <div className="space-y-2">
                            <div className="flex flex-wrap gap-1">
                              ...
                              {article.hasLivescore && (
                                <Badge variant="info" icon={TrendingUp} size="sm">
                                  Livescore
                                </Badge>
                              )}
                            </div>
                        ...
                        <td className="py-4 px-4">
                          <div className="flex flex-col gap-2">
                            ...
                            {article.isLive ? (
                              <Link 
                                href={`/ghanascore/edit-live-article/${article._id}`} 
                                target="_blank"
                              ...
                              >
                                <button
                                  ...
                                  title="Edit Live Article (opens in new tab)"
                                >
                              ...
                              <Link href={`/ghanascore/edit-article/${article._id}`} passHref>
                                <button
