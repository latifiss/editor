...
export default function EditFeaturePage() {
  ...
  useEffect(() => {
    console.log('=== Loading AfrobeatsRep Feature Data ===');
    
    if (featureData?.data && !isInitialized) {
      ...
      console.log('✅ Feature loaded successfully:', {
        ...
        tags: feature.tags,
        contentExists: !!feature.content,
        contentLength: feature.content?.length || 0
      });
      ...
      if (feature.category) {
        console.log('Setting category:', feature.category);
        setCategory({ id: feature.category, label: feature.category });
      } else {
        console.warn('⚠️ No category found in feature');
      }
      
      if (feature.tags) {
        if (Array.isArray(feature.tags)) {
          console.log('Setting tags:', feature.tags);
      ...
      if (feature.image_url) {
        console.log('Setting image URL:', feature.image_url);
      ...
      if (feature.content && typeof feature.content === 'string') {
        console.log('Feature content found, length:', feature.content.length);
        
        setTimeout(() => {
          if (editorRef.current) {
            console.log('Setting content to editor via ref');
            try {
              editorRef.current.setContent(feature.content);
  ...
  useEffect(() => {
    if (featureError && !hasErrorBeenHandled && featureId) {
      console.error('Feature loading error:', featureError);
      notify('Failed to load feature. Please try again.', 'error');
      setHasErrorBeenHandled(true);
      
      setTimeout(() => {
        router.push('/afrobeatsrep/features');
      }, 2000);
    }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      console.log('✅ Feature update successful:', result);
            ...
            <div className="w-full space-y-6">
              <div className="space-y-2">
                ...
                <div className="w-full [&>div]:!w-full">
                  <Textarea
                    id="title"
                    placeholder="Enter feature title or heading"
                    value={title}
